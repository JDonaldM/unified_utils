...
def deconvolved_data_loader(redshift, split, kmin=0.001, kmax=0.4, path_to_repo="/mnt/lustre/jdonaldm/unified_analysis/"):
    '''
    Utility function for generating deconvolved power spectrum and covariance matrix. 


    Args:
        redshift (float) : Redshift of data to load. Can be on of ``[0.096, 0.38, 0.61, 1.52]``.
        split (str) : Hemisphere. Can be either ``NGC`` or ``SGC``.
        kmin: minimium wavenumber to consider
        kmax: maximal wavenumber to consider
        path_to_repo (str) : Path to repo. Note that all data must be stored in a directory
         with name ``data``.
    Returns:
        kbins_fit (array) : The k-bins. Has shape (nki,).
        pk_data_fit (array) : The selected multipoles. Has shape (3*nki,).
        cov_fit (array): Covariance matrix. Has shape (3*nki, 3*nki).
        window (array) : Window function matrix. Has shape (200, 200).
        range_selection (array): Array of boolean elements that cut the
         origonal data. Has shape (nkj,). 
    '''
    ...
    if redshift == 0.38 or redshift ==0.61:
        datafile = f"{path_to_repo}/ps1D_BOSS_DR12_{split}_{BOSS_z_dict[redshift]}_COMPnbar_TSC_700_700_700_400_renorm.dat"
        windowfile = f"{path_to_repo}/W_BOSS_DR12_{split}_{BOSS_z_dict[redshift]}_V6C_1_1_1_1_1_10_200_200_averaged_v1.matrix"
        covfile = f"{path_to_repo}/C_2048_BOSS_DR12_{split}_{BOSS_z_dict[redshift]}_V6C_1_1_1_1_1_10_200_200_prerecon_deconvolved.matrix"


        try: 
            test = Path(covfile)
            test.resolve(strict = True)
        except FileNotFoundError:
            print("No covariance matrixfrom deconvolved power spectrum. \n Calculating new one...")
            deconvolved_cov = False
            datafiles = f"{path_to_repo}/Patchy_V6C_BOSS_DR12_{split}_{BOSS_z_dict[redshift]}_2/*"

    elif redshift == 1.52: 
        datafile = f"{path_to_repo}/ps1D_eBOSS_DR16_QSO_{split}_TSC_0.8_2.2_{eBOSS_split_data_dict[split]}_{eBOSS_split_data_dict[split]}_{eBOSS_split_data_dict[split]}_400_renorm.dat"
        windowfile = f"{path_to_repo}/W_eBOSS_DR16_QSO_{split}_1_1_1_1_1_10_200_200_averaged_v1.matrix"
        covfile = f"{path_to_repo}/C_{eBOSS_split_cov_dict[split]}_eBOSS_DR16_QSO_{split}_1_1_1_1_1_10_200_200_prerecon_deconvolved.matrix"
        try: 
            test = Path(covfile)
            test.resolve(strict = True)
        except FileNotFoundError:
            print("No covariance matrixfrom deconvolved power spectrum. \n Calculating new one...")
            deconvolved_cov = False
            datafiles = f"{path_to_repo}/EZmocks_eBOSS_DR16_QSO_{split}/*"

    else: 
        datafile = f"{path_to_repo}/ps1D_6dFGS_DR3_TSC_600_600_600_400_renorm.dat"
        windowfile = f"{path_to_repo}/W_6dFGS_DR3_1_1_1_1_1_10_200_200_averaged_v1.matrix"
        covfile = f"{path_to_repo}/C_600_6dFGS_DR3_1_1_1_1_1_10_200_200_prerecon_deconvolved.matrix"
        try: 
            test = Path(covfile)
            test.resolve(strict = True)
        except FileNotFoundError:
            print("No covariance matrixfrom deconvolved power spectrum. \n Calculating new one...")
            deconvolved_cov = False
            datafiles = f"{path_to_repo}/COLA_6dFGS_DR3/*"

    #Windowfunction
    W = pk_tools.read_matrix(windowfile)
    Winv = np.linalg.inv(W)
    W_fit = W[np.ix_(np.repeat(pole_selection , 40), np.repeat(pole_selection , 40))]

    
    #Covariance matrix
    if deconvolved_cov == False:
        cov_deconvolved = 0
        pk_deconvolved = []
        print("using mock data to calculate new cov matrix")
        print("mock data loaded:",datafiles)
        for file_i in glob.glob(datafiles):
            pk_data_dict_i = pk_tools.read_power(file_i, combine_bins=10)
            kbins_i, pk_data_vector_i = pk_tools.dict_to_vec(pk_data_dict_i)
            pk_data_vector_deconvolved_i = np.matmul(Winv, pk_data_vector_i)
            pk_deconvolved.append(pk_data_vector_deconvolved_i)
        cov_deconvolved = np.cov(np.stack(pk_deconvolved), rowvar = False)
        cov_deconvolved_fit = cov_deconvolved[np.ix_(np.repeat(pole_selection , 40), np.repeat(pole_selection , 40))]
    else:
        cov_deconvolved_fit = pk_tools.read_matrix(covfile)

    #Deconvolved powerspectrum
    pk_data_dict = pk_tools.read_power(datafile,combine_bins=10)
    kbins, pk_data_vector = pk_tools.dict_to_vec(pk_data_dict)
    pk_data_vector_deconvolved = np.matmul(Winv, pk_data_vector)
    pk_data_fit = pk_data_vector_deconvolved[np.repeat(pole_selection , 40)]
    kbins_fit = kbins[np.repeat(pole_selection , 40)]
    kbins_fit = kbins_fit[:40]
    ...
    kbins_fit = kbins_fit[range_selection].__array__()
    pk_data_fit = pk_data_fit[np.concatenate(sum(pole_selection)*[range_selection])].__array__()
    cov_fit = cov_deconvolved_fit[np.ix_(np.concatenate(sum(pole_selection)*[range_selection]), np.concatenate(sum(pole_selection)*[range_selection]))].__array__()
    window_fit = W_fit[np.ix_(np.concatenate(sum(pole_selection)*[range_selection]), np.concatenate(sum(pole_selection)*[range_selection]))].__array__()

    return kbins_fit, pk_data_fit, cov_fit, window_fit ,range_selection  
    
def deconvolved_mock_loader(redshift, split, kmin=0.001, kmax=0.4, path_to_repo="/mnt/lustre/jdonaldm/unified_analysis/"):
    '''
    Utility function for generating deconvolved power spectrum and covariance matrix from mock data
    ...
    Returns:
        kbins_fit (array) : The k-bins. Has shape (nki,).
        pk_data_fit (array) : The selected multipoles. Has shape (3*nki,).
        std (array): Standard deviation of the average of the deconvolved mock power spectrum (nki,)
        cov_fit (array): Covariance matrix. Has shape (3*nki, 3*nki).
        window (array) : Window function matrix. Has shape (200, 200).
        range_selection (array): Array of boolean elements that cut the
         origonal data. Has shape (nkj,). 
    ...
    if deconvolved_cov == False:
        ...
        for file_i in glob.glob(datafiles):
            ...
            kbins_i, pk_data_vector_i = pk_tools.dict_to_vec(pk_data_dict_i)
            pk_data_vector_deconvolved_i = np.matmul(Winv, pk_data_vector_i)
            pk_data_fit += pk_data_vector_deconvolved_i[np.repeat(pole_selection , 40)]
            pk_deconvolved.append(pk_data_vector_deconvolved_i)
            kbins_fit = kbins_i[np.repeat(pole_selection , 40)]
            kbins_fit = kbins_fit[:40]
        std = np.std(np.stack(pk_deconvolved), axis = 0)
        cov_deconvolved = np.cov(np.stack(pk_deconvolved), rowvar = False)
        cov_deconvolved_fit = cov_deconvolved[np.ix_(np.repeat(pole_selection , 40), np.repeat(pole_selection , 40))]
        pk_data_fit = pk_data_fit/len(glob.glob(datafiles))
    ...
    return kbins_fit, pk_data_fit, std, cov_fit, window_fit ,range_selection  
